...
module Protobuf
  module Nats
    class Send # < ::Protobuf::Rpc::Connectors::Base
      # attr_reader :app
      #
      ...
      def call(env)
        env.encoded_response = send_request(
          env.service_name.constantize,
          env.method_name,
          env.encoded_request
        )
      ...
      def send_request(service, method, req)
        # This will ensure the client is started.
        ::Protobuf::Nats.start_client_nats_connection
        ...
        ::ActiveSupport::Notifications.instrument "client.request_duration.protobuf-nats" do
          send_request_through_nats(service, method, req)
        end
      end

      def send_request_through_nats(service, method, req)
        retries ||= 3
        nack_retry ||= 0

        loop do
          # XXX bring this back: setup_connection
          request_options = {:timeout => response_timeout, :ack_timeout => ack_timeout}
          subscription_key = cached_subscription_key(service, method)
          @response_data = nats_request_with_two_responses(subscription_key, req, request_options)
          case @response_data
          when :ack_timeout
      ...
      def cached_subscription_key(service, method)
        method_name_cache = self.class.subscription_key_cache[service] ||= {}
        method_name_cache[method] ||= begin
          ::Protobuf::Nats.subscription_key(service, method)
        end
      end

      # TODO Test with JRuby
      #
...
::ActiveSupport.on_load(:protobuf_rpc_client_middleware) do |rpc|
  rpc.client_middleware.insert_after(::Protobuf::Rpc::Middleware::Client::RequestEncoder, ::Protobuf::Nats::Send)
end
